...
class Spider():
    def __init__(self):
        ...
        self.log_dir = self.mkdir(os.path.join(os.path.dirname(__file__), 'log'))#把目录合成为一个路径 并mkdir
        self.video_dir = self.mkdir(os.path.join(os.path.dirname(__file__), 'video'))#同上
    ...
    def __build_logger(self):
        filename = os.path.join(self.log_dir, 'spider.log')#log目录+sp。log
        logger = logging.getLogger(__name__)#获取log日志对象，名字为root（默认名）
